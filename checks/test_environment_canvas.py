...
from gui.layout_repair import auto_repair_targets_config
...
class EnvironmentCanvasTest(unittest.TestCase):
    ...
    def test_complete_scene_and_lossless_initial_load(self):
        ...
        self.assertEqual({k[0] for k in controls(self.c)}, {'boundary','agent','seed','column','wall','fixed_rect','door'})
    ...
    def test_hidden_layers_preserve_geometry_and_graph(self):
        display={'column':(False,0),'shear_wall':(False,0),'agent':(True,65)}
    ...
    def test_auto_repair_helper_handles_traffic_core(self):
        repaired, repairs, info = auto_repair_targets_config(self.c, 42)
    ...
    def test_column_and_wall_move_without_resizing(self):
        updated=self.move(('column','c','center'),.2,.1)
        c=updated['ExistingBuilding']['fixed_objects'][0]
        self.assertAlmostEqual(c['center'][0],5.2);self.assertAlmostEqual(c['center'][1],5.1)
        self.assertAlmostEqual(c['size'][0],.4);self.assertAlmostEqual(c['size'][1],.6)
        updated=self.move(('wall','w','center'),1,0)
        w=updated['ExistingBuilding']['fixed_objects'][1]
        self.assertEqual(w['start'],[11,5]);self.assertEqual(w['end'],[17,8])
        self.assertEqual(w['type'],'shear_wall')
    ...
    def test_corner_crossing_rejected(self):
        with self.assertRaises(ValueError): self.move(('fixed_rect','traffic_1','sw'),10,0)
