...
def main():
    ...
    pool=candidate_partitions(replace(p,search_settings={**p.search_settings,'compact_lobby':False}))
    seeds=sorted(pool,key=lambda item:item[1]['corridor_area'])[:2]
    summary=[]
    for spacing in [p.profile.min_door_spacing,1.2]:
        ...
        if not candidates: raise RuntimeError(f'No verified lobby at spacing {spacing}')
        r,q=min(candidates,key=lambda item:item[1]['corridor_area'])
        out=root/f'results2/floor_partition/lobby_spacing_{spacing:g}_20260918'
