...
def render_structure_editor(config, config_id):
    ...
    if not render_boundary_editor(config,config_id):
        return False
    st.caption('单位：米。柱按中心点及 X/Y 尺寸定义；墙线左右以起点→终点方向为准，左右厚度独立输入；交通核区域/保留交通空间按矩形范围定义。表格可增删行。')
    columns, walls, zones, other = parameter_rows(config['ExistingBuilding'].get('fixed_objects', []))
    ...
    with st.container():
        ...
        zone_df = st.data_editor(pd.DataFrame(zones, columns=['id', 'type', 'x1', 'y1', 'x2', 'y2']),
            num_rows='dynamic', use_container_width=True, key=f'{config_id}_zones_{revision}',
            column_config={
                'type': st.column_config.SelectboxColumn('区域属性', options=['traffic_core', 'retained_circulation', 'fixed'], required=True),
                **{k: st.column_config.NumberColumn(v, format='%.4f') for k, v in
                   dict(x1='左下 X', y1='左下 Y', x2='右上 X', y2='右上 Y').items()}})
        st.caption('shear_wall＝剪力墙；load_bearing_wall＝承重墙。剪力墙中心线无缝闭合时自动识别核心筒；交通核区域与保留交通空间可直接录入。它们都会整体保留，不允许功能空间进入。')
    try:
        objects = build_structures(column_df.to_dict('records'), wall_df.to_dict('records'), zone_df.to_dict('records'), other)
